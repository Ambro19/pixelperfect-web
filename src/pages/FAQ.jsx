...
//
// ✅ UPDATED (July 2026):
// - URLs inside FAQ answers are now clickable links (open in new tab).
//   Implemented via linkify() — automatically converts any http(s) URL in
//   an answer string into an <a> tag. Applies to the status page link,
//   documentation link, and any future URLs added to answers.
// - Screenshot storage period corrected: 30 days → 7 days (two answers:
//   "How long are screenshots stored?" and "Do you store the websites I
//   screenshot?") — now matches actual R2 lifecycle rules.
//
// ✅ UPDATED (March 2026):
...
// ── ✅ NEW (July 2026): linkify — turn URLs in answer text into real links ──
// Splits the answer on http(s) URLs and wraps each one in an <a> tag.
// Trailing punctuation (e.g. a period ending the sentence) is kept outside
// the link so hrefs stay clean.
  ...
  const faqCategories = [
    ...
    },
    {
      ...
      questions: [
        ...
        {
          // ✅ FIX (July 2026): storage period corrected 30 → 7 days
          q: 'How long are screenshots stored?',
          a: 'Screenshots are stored for 7 days by default. After 7 days, they are automatically deleted. You can delete screenshots earlier through the API or dashboard if needed.'
        },
        {
    ...
    },
    {
      ...
      questions: [
        ...
        },
        {
          ...
          a: 'We only store the screenshot images themselves, not the underlying HTML or source code. Screenshots are stored for 7 days then automatically deleted.'
        },
        {
                  ...
                  <div key={qIdx} className="bg-white rounded-lg border border-gray-200 overflow-hidden">
                    ...
                    {isOpen && (
                      <div className="px-6 py-4 border-t border-gray-200 bg-gray-50">
                        {/* ✅ NEW (July 2026): answers rendered through linkify()
                            so URLs become clickable links (new tab) */}
                        <p className="text-gray-700 leading-relaxed">{linkify(item.a)}</p>
                      </div>
                    )}
                  </div>
                );
