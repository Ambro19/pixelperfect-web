...
import { useSubscription } from '../contexts/SubscriptionContext';   // ✅ NEW
import { getDisplayEmail, getDisplayName } from '../utils/userDisplay';
import PixelPerfectLogo from '../components/PixelPerfectLogo';

// ✅ FIX: Single source of truth for the API base URL.
// Must match lib/api.js resolveBaseURL() logic so relative screenshot_urls
// (e.g. "/screenshots/abc.png") are correctly prefixed with the backend origin,
// not the frontend origin (which would produce localhost:3000/screenshots/... on mobile).
function resolveApiBase() {
  ...
  if (typeof window !== 'undefined') {
    ...
    // ✅ LAN/mobile: e.g. 192.168.1.185:3000 → API at 192.168.1.185:8000
...
// "/screenshots/abc.png". Opening these as <a href> would resolve to the
// *frontend* origin (wrong). We prefix with the backend API base instead.
...
const normalizeItem = (raw, idx) => {
  ...
  const fileFormat = get(raw, 'format', 'file_format', 'ext', 'extension') || 'png';
  const timestamp = get(raw, 'created_at', 'timestamp', 'captured_at') || new Date().toISOString();
  const targetUrl = get(raw, 'url', 'target_url', 'website_url');

  // ✅ FIX: Resolve screenshot_url to a fully-qualified URL so "View Screenshot"
  // works on any device (mobile, LAN, production) — not just the dev machine.
  const rawScreenshotUrl = get(raw, 'screenshot_url', 'storage_url', 'file_url');
  const resolvedScreenshotUrl = resolveScreenshotUrl(rawScreenshotUrl);
  ...
  return {
    ...
    // ✅ Fully-qualified URL (R2, CDN, or backend-prefixed relative path)
    ...
    >
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 border border-gray-100">
        ...
        <p className="text-sm text-gray-600 text-center mb-3">
          This permanently removes the screenshot and its image file. This action
          cannot be undone.
        </p>
 
        {/* ✅ NEW (Aug 2026): state the quota behaviour BEFORE the user commits.
            Usage measures what was captured, not what is still stored — the
            capture cost is spent the moment the page is rendered. Without this
            note, a user deleting screenshots to "free up" quota sees no change
            in their counter and reasonably reads it as a bug. */}
        <div className="flex items-start gap-2 text-xs text-gray-600 bg-amber-50 border border-amber-200 rounded-lg px-3 py-2 mb-4 text-left">
          <svg
          ...
          <span>
            <strong className="text-amber-800">This does not restore quota.</strong>{' '}
            Your monthly usage counts screenshots you captured, not screenshots you
            still have stored — so deleting won't free up allowance.
          </span>
        </div>
 
        {item?.url && (
          <p className="text-xs text-gray-500 text-center truncate mb-5 bg-gray-50 px-3 py-2 rounded-lg">
            🌐 {item.url}
          </p>
        )}
        ...
        <div className="flex gap-3">
          <button
            onClick={onCancel}
            disabled={isDeleting}
            className="flex-1 px-4 py-2.5 rounded-xl border border-gray-300 text-gray-700 font-medium
          ...
          <button
            onClick={onConfirm}
            disabled={isDeleting}
            className="flex-1 px-4 py-2.5 rounded-xl bg-red-600 hover:bg-red-700 text-white font-medium
                       transition-colors disabled:opacity-50 text-sm flex items-center justify-center gap-2"
          >
            {isDeleting ? (
              <>
...
export default function History() {
  ...
  const { tier } = useSubscription();                    // ✅ NEW
  const isFreeTier = (tier || 'free').toLowerCase() === 'free';   // ✅ NEW
  ...
  // DELETE HANDLERS
  // ============================================================
  const handleDeleteClick = useCallback((item) => setDeleteTarget(item), []);
  const handleDeleteCancel = useCallback(() => setDeleteTarget(null), []);

  const handleDeleteConfirm = useCallback(async () => {
    if (!deleteTarget || !token) return;

    const rawId = deleteTarget.id;

    if (!rawId || (typeof rawId === 'string' && rawId.startsWith('generated-'))) {
      toast.error('Cannot delete: this item has no server ID. Please refresh the page.', { duration: 5000 });
    ...
    if (!idStr) {
      toast.error('Cannot delete: missing screenshot ID. Please refresh.', { duration: 5000 });
      if (mountedRef.current) setDeleteTarget(null);
      return;
    }
    ...
    try {
      const response = await fetch(`${API_BASE_URL}/api/v1/screenshots/${encodeURIComponent(idStr)}`, {
        method: 'DELETE',
        headers: {
      ...
      if (!response.ok) {
        ...
        try {
          ...
          if (data.detail) {
            ...
            errorMessage = 'Screenshot not found — it may have already been deleted.';
          } else if (response.status === 403) {
            errorMessage = 'You do not have permission to delete this screenshot.';
          } else if (response.status === 405) {
            errorMessage = 'Delete endpoint not available — please redeploy the backend.';
          } else if (response.status === 422) {
            errorMessage = 'Server rejected the screenshot ID format.';
          }
        } catch { /* JSON parse failed, use status-based message */ }
        throw new Error(errorMessage);
      }

      setItems((prev) => prev.filter((item) => String(item.id) !== String(rawId)));
      cache.delete(username);
      toast.success('Screenshot deleted successfully', { duration: 3000 });
      debug('Deleted screenshot id:', idStr);
    } catch (err) {
      console.error('❌ Delete failed:', err);
      toast.error(`Failed to delete: ${err.message || 'Unknown error'}`, { duration: 5000 });
    } finally {
      if (mountedRef.current) {
        setDeletingId(null);
  ...
  const filtered = useMemo(() => {
    ...
    return items.filter((item) => {
      ...
      switch (active) {
        ...
        case 'jpeg': return format === 'jpeg' || format === 'jpg';
        case 'webp': return format === 'webp';
  ...
  const counts = useMemo(() => {
    ...
    items.forEach((item) => {
      const format = (item.format || '').toLowerCase();
      if (format === 'png') cats.png++;
      else if (format === 'jpeg' || format === 'jpg') cats.jpeg++;
      else if (format === 'webp') cats.webp++;
      else if (format === 'pdf') cats.pdf++;
    });
              ...
              <div className="ml-3 flex-1">
                ...
                <p className="text-red-700 text-sm leading-relaxed mb-3">
                  {connectionError.includes('missing columns')
                    ? 'The database needs to be updated. Run: python migrate_database.py'
            ...
            <div className="py-16 text-center">
              ...
              <p className="text-gray-600 mb-6 max-w-md mx-auto">
                {items.length === 0
                  ...
                  : `You haven't captured any ${active} screenshots yet.`}
              </p>
              <div className="flex flex-col sm:flex-row gap-3 justify-center">
                ...
                <div className="text-6xl mb-4">{active === 'pdf' && isFreeTier ? '🔒' : '📸'}</div>
                <h3 className="text-xl font-semibold text-gray-900 mb-2">
                  {items.length === 0
                  ? 'No screenshots yet'
                  : active === 'pdf' && isFreeTier
                  ? 'PDF capture is a Pro feature'
                  : `No ${active} screenshots`}
                </h3>
                <p className="text-gray-600 mb-6 max-w-md mx-auto">
                  {items.length === 0
                    ? 'Start capturing screenshots to see your history here.'
                    : active === 'pdf' && isFreeTier
                    ? "Your Free plan captures PNG, JPEG and WebP. PDF output is available on Pro, Business and Premium — useful for multi-page reports and archiving full documents."
                    : `You haven't captured any ${active} screenshots yet.`}
                </p>
                <div className="flex flex-col sm:flex-row gap-3 justify-center">
                  {active === 'pdf' && isFreeTier ? (
                    ...
                    <div className="flex-1 min-w-0">
                      ...
                      <div className="mt-3 flex flex-wrap gap-2">
                        {!item.screenshot_url ? (
                          ...
                          <a
                            ...
                            className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-blue-50 text-blue-700
                                       border border-blue-200 rounded-lg hover:bg-blue-100 text-sm
                        ...
                        <button
                          ...
                          disabled={!!deletingId}
                          className="inline-flex items-center gap-1.5 px-3 py-1.5 bg-red-50 text-red-700
