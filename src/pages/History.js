...
//     // ✅ FIX: FastAPI route is `screenshot_id: int` — path param MUST be a valid integer.
//     // normalizeItem stores id from the server response which might be a numeric string.
//     // The generated-* fallback means the item has no real DB record → reject early.
//     if (typeof rawId === 'string' && rawId.startsWith('generated-')) {
//       toast.error('Cannot delete: this item has no server ID. Please refresh the page.', { duration: 5000 });
//       if (mountedRef.current) setDeleteTarget(null);
//       return;
//     }
...
//       toast.error(`Cannot delete: invalid screenshot ID "${rawId}". Please refresh.`, { duration: 5000 });
//       if (mountedRef.current) setDeleteTarget(null);
//       return;
//     }
...
//     try {
//       const response = await fetch(`${API_BASE_URL}/api/v1/screenshots/${numericId}`, {
//         method: 'DELETE',
//         headers: {
//           Authorization: `Bearer ${token}`,
//           'Content-Type': 'application/json',
...
//               errorMessage = data.detail;
//             } else if (Array.isArray(data.detail)) {
//               // 422 Unprocessable Entity: [{loc, msg, type}, ...]
//               errorMessage = data.detail.map((e) => e.msg || JSON.stringify(e)).join(', ');
//             } else {
//               errorMessage = JSON.stringify(data.detail);
//             }
...
//       // ✅ Optimistically remove from UI + invalidate cache
//       // Match by rawId (original value) to handle both number and string ids
//       setItems((prev) => prev.filter((item) => String(item.id) !== String(rawId)));
//       cache.delete(username);

//       toast.success('Screenshot deleted successfully', { duration: 3000 });
//       debug('Deleted screenshot id:', numericId);
...
//     try {
//       const response = await fetch(`${API_BASE_URL}/api/v1/screenshots/${id}`, {
//         method: 'DELETE',
//         headers: {
//           Authorization: `Bearer ${token}`,
//           'Content-Type': 'application/json',
...
//         try {
//           const data = await response.json();
//           // ✅ FIX: FastAPI returns detail as string (404/403) OR array of objects (422)
//           if (data.detail) {
//             if (typeof data.detail === 'string') {
//               errorMessage = data.detail;
//             } else if (Array.isArray(data.detail)) {
...
//                   key={item.id}
//                   className={`p-6 transition-colors duration-150 ${
//                     deletingId === item.id ? 'opacity-40 pointer-events-none' : 'hover:bg-gray-50'
//                   }`}
//                 >
//                   <div className="flex items-start gap-4">
//                     <div className="text-3xl flex-shrink-0">{typeToIcon(item.type, item.format)}</div>
