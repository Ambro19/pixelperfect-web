  ...
  return (
    <div className="fixed inset-0 z-50 flex items-center justify-center p-4">
      ...
      <div className="relative bg-white rounded-2xl shadow-2xl max-w-md w-full p-6 border border-gray-100">
        ...
        <h3 className="text-lg font-bold text-gray-900 text-center mb-2">Delete Screenshot?</h3>
...
// // UPDATED: February 2026 - Added Delete Screenshot functionality
...
//     // ✅ ROOT-CAUSE FIX: PostgreSQL uses UUID primary keys (e.g. "c86e1436-bd80-49ad-ae1c-000a43d70738"),
//     // NOT integers. SQLite (local dev) may use integers. Both are valid — accept either.
//     // Only reject the client-side generated-* fallback which has no DB record.
//     if (!rawId || (typeof rawId === 'string' && rawId.startsWith('generated-'))) {
//       toast.error('Cannot delete: this item has no server ID. Please refresh the page.', { duration: 5000 });
//       if (mountedRef.current) setDeleteTarget(null);
//       return;
...
//     // Store as raw value for UI dim comparison (works for both int and UUID)
//     setDeletingId(rawId);

//     try {
//       // encodeURIComponent handles UUIDs safely in the path segment
//       const response = await fetch(`${API_BASE_URL}/api/v1/screenshots/${encodeURIComponent(idStr)}`, {
//         method: 'DELETE',
//         headers: {
//           Authorization: `Bearer ${token}`,
...
//         try {
//           const data = await response.json();
//           // FastAPI returns detail as string (404/403) OR array of objects (422)
//           if (data.detail) {
//             if (typeof data.detail === 'string') {
//               errorMessage = data.detail;
//             } else if (Array.isArray(data.detail)) {
...
//             errorMessage = 'Delete endpoint not available — please redeploy the backend.';
//           } else if (response.status === 422) {
//             errorMessage = 'Server rejected the screenshot ID format. Check backend DELETE route accepts string IDs.';
//           }
//         } catch {
...
//       // Optimistically remove from UI + invalidate cache
//       setItems((prev) => prev.filter((item) => String(item.id) !== String(rawId)));
//       cache.delete(username);
...
//                   key={item.id}
//                   className={`p-6 transition-colors duration-150 ${
//                     deletingId === item.id || String(deletingId) === String(item.id) ? 'opacity-40 pointer-events-none' : 'hover:bg-gray-50'
//                   }`}
//                 >
//                   <div className="flex items-start gap-4">
//                     <div className="text-3xl flex-shrink-0">{typeToIcon(item.type, item.format)}</div>
...
//                       {/* ✅ Action buttons: View + Delete side by side */}
//                       <div className="mt-3 flex flex-wrap gap-2">
//                         {item.screenshot_url && (
//                           <a
//                             href={item.screenshot_url}
