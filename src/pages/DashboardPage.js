...
//
//   Fix:
//   • Mount auto-refresh → refreshSubscriptionStatus(false) — fast DB read
//   • Focus/visibility auto-refresh → refreshSubscriptionStatus(false) — no Stripe
//   • Manual "Refresh Usage" button → refreshSubscriptionStatus(true) — Stripe sync
//
//   Stripe is now called ONLY when the user deliberately clicks the button.
// ============================================================================
...
export default function DashboardPage() {
  ...
  // ✅ FIX: was calling refreshSubscriptionStatus() with no arg → forceSync=true
  //         → Stripe call on every page load. Now explicitly passes false.
  useEffect(() => {
  ...
  // ✅ FIX: was calling refreshSubscriptionStatus() with no arg → forceSync=true
  //         → Stripe call on every tab switch. Now explicitly passes false.
  //   5-second debounce prevents rapid-fire calls from multiple events.
  useEffect(() => {
    if (!isAuthenticated || !refreshSubscriptionStatus) return;
    ...
    const trigger = () => {
      ...
      refreshSubscriptionStatus(false)            // ← sync=0, no Stripe
  ...
  // ✅ This is the ONLY place where forceSync=true (sync=1) is used.
  //    Stripe is never called automatically; only when the user deliberately
  //    requests it via the "Refresh Usage" button.
...
//   // ─── Debounce ref: prevents focus/visibility from hammering the API ──────────
//   // Root cause of the Stripe flood visible in server logs (2 calls every 30s):
//   // every tab switch was triggering a full Stripe sync. Now auto-refreshes use
//   // sync=0 (fast DB read only), and only the manual button uses sync=1 (Stripe).
//   const debounceRef = useRef(null);

//   // ─── Auto-refresh on mount: fast DB read (no Stripe sync) ───────────────────
//   useEffect(() => {
//     if (!isAuthenticated || !refreshSubscriptionStatus) return;
//     let cancelled = false;
//     const run = async () => {
...
//     run();
//     return () => { cancelled = true; };
//     // Intentionally omit refreshSubscriptionStatus from deps — it may be an
//     // unstable reference that would cause an infinite re-render loop.
//     // eslint-disable-next-line react-hooks/exhaustive-deps
//   }, [isAuthenticated]);

//   // ─── Refresh on tab focus / visibility — debounced to 5s minimum gap ─────────
//   // Prevents hammering the API every time the user switches tabs.
//   useEffect(() => {
//     if (!isAuthenticated || !refreshSubscriptionStatus) return;
//     const trigger = () => {
//       if (debounceRef.current) return;            // still within cooldown
//       debounceRef.current = setTimeout(() => {
//         debounceRef.current = null;
//       }, 5000);
//       refreshSubscriptionStatus()
...
//   // ─── Manual refresh — triggers Stripe sync (sync=1) via context ──────────────
//   const handleManualRefresh = async () => {
//     setIsRefreshing(true);
//     try {
//       await refreshSubscriptionStatus();
