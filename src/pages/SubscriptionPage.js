...
// ========================================
// SUBSCRIPTION PAGE - FIXED NAVIGATION
// ========================================
// File: frontend/src/pages/SubscriptionPage.js
// Author: OneTechly
...
//
// ✅ FIXES:
// - Dynamic success/cancel URLs preserve current origin (LAN vs localhost)
// - No more mixed localhost/LAN navigation history
// - Stripe redirects work correctly on any host
// ========================================
...
export default function SubscriptionPage() {
  ...
  const getCurrentOrigin = () => {
    ...
    return window.location.origin; // Preserves LAN IP or localhost
  };

  // Extract plan from URL query param
  const queryPlan = new URLSearchParams(location.search).get('plan');

  useEffect(() => {
    if (queryPlan && ['pro', 'business', 'premium'].includes(queryPlan.toLowerCase())) {
      // Auto-trigger upgrade flow if coming from pricing page
      handleUpgrade(queryPlan.toLowerCase());
    }
  }, [queryPlan]);
  ...
  const plans = {
    monthly: [
      ...
      },
      {
        ...
        price: 49,
        period: '/month',
        lookup_key: 'pixelperfect_pro_monthly',
        popular: true,
        features: [
        ...
        current: subscription?.tier === 'pro' && subscription?.interval === 'month',
      },
      {
        id: 'business',
        name: 'Business',
        price: 199,
        period: '/month',
        lookup_key: 'pixelperfect_business_monthly',
        features: [
        ...
        current: subscription?.tier === 'business' && subscription?.interval === 'month',
      },
      {
        id: 'premium',
        name: 'Premium',
        price: 499,
        period: '/month',
        lookup_key: 'pixelperfect_premium_monthly',
        features: [
          'Unlimited screenshots',
          'All Business features',
          'White-label options',
        ...
        current: subscription?.tier === 'premium' && subscription?.interval === 'month',
      },
    ],
    annual: [
      {
      ...
      },
      {
        ...
        price: 490,
        period: '/year',
        lookup_key: 'pixelperfect_pro_annual',
        popular: true,
        ...
        current: subscription?.tier === 'pro' && subscription?.interval === 'year',
      },
      {
        id: 'business',
        name: 'Business',
        price: 1990,
        period: '/year',
        lookup_key: 'pixelperfect_business_annual',
        ...
        current: subscription?.tier === 'business' && subscription?.interval === 'year',
      },
      {
        id: 'premium',
        name: 'Premium',
        price: 4990,
        period: '/year',
        lookup_key: 'pixelperfect_premium_annual',
        ...
        current: subscription?.tier === 'premium' && subscription?.interval === 'year',
      },
    ],
  };
  ...
  const handleUpgrade = async (planId) => {
    ...
    const selectedPlan = plans[billingCycle].find((p) => p.id === planId);
    if (!selectedPlan || !selectedPlan.lookup_key) {
    ...
    try {
      // ✅ CRITICAL FIX: Use dynamic origin for success/cancel URLs
      const currentOrigin = getCurrentOrigin();
      
      const response = await apiPostJson('/create-checkout-session', {
        lookup_key: selectedPlan.lookup_key,
        success_url: `${currentOrigin}/subscription?success=true`,
        cancel_url: `${currentOrigin}/subscription?canceled=true`,
      });
  ...
  const handleCancelSubscription = async () => {
    ...
    try {
      await apiPostJson('/cancel-subscription', {});
  ...
  useEffect(() => {
    ...
    if (params.get('success') === 'true') {
      toast.success('🎉 Subscription activated! Welcome to PixelPerfect Pro!');
      refreshSubscription();
      // Clean URL
    ...
    <div className="min-h-screen bg-gray-50">
      ...
      <main className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
            ...
            >
              ...
              <div className="p-8">
                ...
                <div className="mb-6">
                  ...
                  {plan.savings && (
                    <p className="text-sm text-green-600 font-semibold mt-1">{plan.savings}</p>
                  )}
                </div>
                ...
                {plan.current ? (
                  <button
                    className="w-full py-3 rounded-lg font-semibold bg-gray-100 text-gray-700 cursor-default"
                  >
                    ✓ Current Plan
                  </button>
                ) : plan.id === 'free' ? (
                  <button
                    ...
                    className="w-full py-3 rounded-lg font-semibold bg-gray-100 text-gray-700 hover:bg-gray-200 transition-colors"
                    disabled={!subscription || subscription.tier === 'free'}
                  >
                    {subscription && subscription.tier !== 'free' ? 'Downgrade to Free' : 'Free Forever'}
                  </button>
                ) : (
                  <button
                  ...
                  >
                    {loadingPlan === plan.id ? 'Loading...' : `Upgrade to ${plan.name}`}
                  </button>
                )}
              </div>
            </div>
        ...
        <div className="mt-20 max-w-3xl mx-auto">
          ...
          <div className="space-y-6">
            ...
            <div className="bg-white rounded-lg p-6 shadow">
              ...
              <p className="text-gray-600">
                Yes, we offer a 14-day money-back guarantee for all paid plans. If you're not
        ...
        <div className="mt-12 text-center text-sm text-gray-500">
          <p>💳 Secure payments processed by Stripe • 🔒 Cancel anytime • ❌ No hidden fees</p>
        </div>
      </main>
    </div>
  );
