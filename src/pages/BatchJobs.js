...
//
// ============================================================================
// ✅ NEW (Sep 2026 — "Clear" button beside "Submit Batch Job")
// ============================================================================
//   Parity with ScreenshotPage.js. Clears the INPUT only — the URL textarea
//   and any uploaded file. Configuration (preset, format, options, remove
//   elements) and the jobs list are left alone, exactly as the single-capture
//   page leaves its configuration alone.
//
//   It also resets the hidden <input type="file">. Without that, picking the
//   SAME file again after clearing fires no onChange event, so the file
//   silently fails to load. The existing "Clear File" button had the same
//   bug and now uses the same reset.
//
// ============================================================================
// ✅ FIX (Sep 2026 — failed items showed a stray "0")
// ============================================================================
//   `{item.file_size && ...}` renders the number 0 when file_size is 0, which
//   is exactly what a failed item carries. That was the lone "0" in front of
//   every "failed" pill. Both numeric fields now use `> 0`.
//
// ============================================================================
// ✅ FIX (Sep 2026 — failed-item errors were truncated and unreadable)
// ============================================================================
//   "⚠ Cannot switch to a different threa..." was cut at 200px and was raw
//   Playwright/greenlet text anyway. friendlyItemError() translates the known
//   engine errors; the raw message stays available in the tooltip for
//   debugging. Messages now wrap instead of truncating.
//
// ✅ NEW (Sep 2026 — wide full-page notice)
//   Full-page captures wider than 2560px are memory-heavy on the server.
//   The form says so before submission instead of after 20 failures.
//
...
//   Entitlement has three states: loading / has access / does not have access.
//
// ✅ FIX (Aug 2026 — Example cards rendered on one line, centred)
// ✅ NEW (Aug 2026 — "Screenshot expired" state, parity with History.js)
// ✅ NEW (Aug 2026): footer action buttons and scroll-to-top
// ✅ UI REFRESH (Aug 2026): in line with ScreenshotPage.js
//
...
// ✅ NEW (Sep 2026): translate capture-engine errors for per-item display.
// These two are server-side failures, not problems with the listed website,
// so the message says so and points at "Retry Failed".
function friendlyItemError(msg) {
  if (!msg) return 'Failed';
  const m = msg.toLowerCase();
  if (m.includes('cannot switch to a different thread')) {
    return 'Capture engine error (not caused by this website). Use Retry Failed.';
  }
  if (m.includes('target page, context or browser has been closed') ||
      m.includes('browser has been closed') || m.includes('target closed')) {
    return 'Capture engine restarted, often because a page was too large at this size. Use Retry Failed.';
  }
  if (m.includes('err_name_not_resolved')) return 'Website address could not be found.';
  if (m.includes('timed out') || m.includes('timeout')) return 'The website took too long to respond.';
  return msg;
}
            ...
            return (
              ...
              >
                ...
                <div className="flex flex-wrap items-center gap-2 pl-8">
                  {/* ✅ FIX (Sep 2026): `> 0`, not truthiness — a 0 would
                      otherwise render as a literal "0". */}
                  {Number(item.file_size) > 0 && (
                    <span className="text-xs text-gray-400">📏 {formatSize(item.file_size)}</span>
                  )}
                  {Number(item.processing_time) > 0 && (
                  ...
                  {viewUrl && jobExpired ? (
                    ...
                    // ✅ FIX (Sep 2026): translated, wrapping, raw text in tooltip.
                    <span
                      className="basis-full sm:basis-auto text-xs text-red-600 leading-relaxed break-words"
                      title={item.message || 'Failed'}
                    >
                      ⚠ {friendlyItemError(item.message)}
                    </span>
                  ) : null}
                </div>
              </div>
...
export default function BatchJobs() {
  ...
  const isWideFullPage = fullPage && width > WIDE_FULL_PAGE_WIDTH && format !== 'pdf';
  ...
  // ✅ NEW (Sep 2026): resets the hidden file input so re-selecting the same
  // file after clearing still fires onChange.
  ...
  // ✅ NEW (Sep 2026): Clear — parity with ScreenshotPage.js. Clears the input
  // (URLs + file); keeps configuration and the jobs list.
  const handleClear = () => {
    setUrlText('');
    clearUploadedFile();
    setDragOver(false);
          ...
          <div className="bg-white border border-emerald-200 rounded-2xl p-4 mb-6 shadow-sm">
            ...
            <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
              {EXAMPLE_BATCHES.map(ex => (
                <button
                  key={ex.name}
                  onClick={() => { setUrlText(ex.urls); clearUploadedFile(); }}
        ...
        <div className={`bg-white rounded-2xl border border-gray-200 shadow-sm p-5 sm:p-6 mb-8 ${tierKnown && !hasAccess ? 'opacity-60 pointer-events-none' : ''}`}>
          ...
          {isWideFullPage && (
            <div className="mb-5 flex items-start gap-3 bg-amber-50 border border-amber-200 rounded-xl px-4 py-3">
              ...
              <p className="text-xs text-amber-800 leading-relaxed">
                <span className="font-semibold">Full-page captures at {width}px wide use a lot of memory.</span>{' '}
                Very long pages can fail at this width, and in a batch that affects every URL.
                For long pages, use Desktop width or turn off full page.
              </p>
            </div>
          )}
          ...
          {/* ✅ NEW (Sep 2026): Submit + Clear row, same geometry as the
              Capture + Clear row on ScreenshotPage.js. */}
          <div className="flex gap-3">
            <button
              onClick={handleSubmit}
              disabled={submitDisabled}
              className={`flex-1 py-4 font-semibold rounded-xl text-base transition-all duration-200
            ...
            <button
              ...
              onClick={handleClear}
              disabled={submitting}
              className="px-6 py-4 border border-gray-300 rounded-xl text-gray-700 font-medium
...
//================================================================================
// ***** IMPORTANT NOTE: DO NOT DELETE THIS FILE YET: Preset Delay in Sec *****
//===============================================================================
