...
export default function Pricing() {
  ...
  const tiers = useMemo(
    () => ["free", "pro", "business", "premium"].map((id) => PRICING_CONFIG.tiers[id]),
    []
  );
        ...
        <div className="max-w-7xl mx-auto px-4 sm:px-6 lg:px-8">
          <div className="flex justify-between items-center h-14 sm:h-16">
            ...
            <nav className="hidden md:flex items-center gap-6">
              ...
              <button onClick={() => navigate("/documentation")} className="text-gray-600 hover:text-gray-900 font-medium">Docs</button>
            </nav>

            <div className="flex items-center gap-2 sm:gap-3">
              {isAuthenticated ? (
                <button onClick={() => navigate("/dashboard")}
                  className="px-4 sm:px-6 py-1.5 sm:py-2 text-sm sm:text-base bg-blue-600 text-white rounded-lg font-semibold hover:bg-blue-700 shadow-sm transition-colors">
                  Dashboard
                </button>
              ) : (
                <>
                  <button onClick={() => navigate("/login")}
                    className="px-3 sm:px-4 py-1.5 text-sm font-medium text-gray-700 hover:text-gray-900 transition-colors">
                    Sign in
                  </button>
                ...
                <div className={`p-6 ${badge ? "pt-10" : ""} flex flex-col flex-grow`}>
                  ...
                  <div className="text-center mb-6">
                    <span className="text-4xl font-bold text-gray-900">
                      ${yearly && tier.id === "premium" ? "4,990"
                        : yearly && tier.id === "business" ? "1,990"
                        : yearly && tier.id === "pro"      ? "490"
                        : price}
                    </span>
                    {tier.id !== "free" && (
                    ...
                    {yearly && tier.id !== "free" && (
                      <div className="text-xs text-green-600 font-medium mt-1">billed annually</div>
                    )}
                  </div>
