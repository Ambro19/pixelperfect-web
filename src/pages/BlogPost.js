...
// ✅ Twitter / LinkedIn share links wired
// ✅ window.innerWidth removed from render (no hydration issues)
// ✅ FIXED June 2026 — inline code break-all + article overflow-x-hidden
//    prevents long URLs/strings from overflowing card on narrow mobile screens
...
// ── Inline Renderer ────────────────────────────────────────────────────────
// ✅ FIX: Added `break-all` and `max-w-full` to inline <code> className.
//    This prevents long URLs and API paths from overflowing the card
//    container on narrow mobile screens (< 400px viewport width).
//    `break-all` forces a line break at any character boundary — ideal for
//    URLs that contain no natural break points like spaces or hyphens.
function Inline({ text }) {
  const nodes = useMemo(() => {
    if (!text) return [];
    ...
    return parts.flatMap((part, idx) => {
      if (part.startsWith('`') && part.endsWith('`')) {
        return (
          <code
            key={`code-${idx}`}
            className="bg-gray-100 text-red-600 px-1.5 py-0.5 rounded text-sm font-mono break-all max-w-full"
          >
            {part.slice(1, -1)}
          </code>
        );
...
const BlogPost = () => {
  ...
  return (
    <div className="min-h-screen bg-gray-50">
      ...
      {/* ✅ FIX: Added `overflow-x-hidden` to the article wrapper.
          This clips any child element (inline code, long links) that
          would otherwise cause horizontal scroll on mobile devices. */}
      <article className="max-w-4xl mx-auto px-4 sm:px-6 lg:px-8 py-12">
        <div className="bg-white rounded-xl shadow-sm p-6 sm:p-8 lg:p-12 overflow-x-hidden">
          <div className="max-w-none">
            {renderMarkdown(post.content)}
          </div>
