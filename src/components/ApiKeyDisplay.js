...
const ApiKeyDisplay = () => {
  ...
  // ✅ FIX (Jul 2026 v2 — registration key handoff):
  // Register.js stores the just-created full key in a one-shot
  // sessionStorage slot (route guards redirect authenticated users off
  // /register before an in-page panel could render). Read it here on
  // mount, delete it IMMEDIATELY so it never lingers, and display it in
  // the one-time "Save this key now" panel.
  useEffect(() => {
    try {
      const pending = sessionStorage.getItem('pp_new_api_key');
      if (pending) {
        sessionStorage.removeItem('pp_new_api_key'); // one-shot: wipe first
        setApiKey(pending);
        console.log('🔑 Registration key handoff received — displaying once');
      }
    } catch {
...
# Using your API key in Java (Java 11+, java.net.http)
import java.net.URI;
import java.net.http.HttpClient;
import java.net.http.HttpRequest;
import java.net.http.HttpResponse;

public class PixelPerfectExample {
    public static void main(String[] args) throws Exception {
        ...
        HttpRequest request = HttpRequest.newBuilder()
            .uri(URI.create("https://api.pixelperfectapi.net/api/v1/screenshot/"))
